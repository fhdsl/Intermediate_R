...
Your turn: access the first list element of `patient` via double brackets [[ ]] or \$ and modify it to a value of your choice. Also, can you access the `visit1`'s first element's second element: "sore throat"?
...
2.  `select()` function. `select(dataframe, column_name)` will give you the column as a 1-column dataframe. Usually recommended when we select multiple columns.
