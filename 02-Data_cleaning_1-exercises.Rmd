...
```{r}
x = read_csv("classroom_data/students.csv", comment = "#")
```
...
Load in data about the following population: **melanoma incidence in WA at the county level for males of all ages, all cancer stages, averaged in the past 5 years.** Store it as a dataframe variable of choice.
...
```{r}
male_melanoma_WA = select(male_melanoma_WA, County, `Age Adjusted Incidence Rate`, `Recent Trend`)

names(male_melanoma_WA) = c("County", "Age_adjusted_incidence_rate", "Recent_trend")
```
